...
string encrypt(string filename)
{
    ...
    //CryptoPP::CBC_Mode< CryptoPP::AES >::Encryption e;e.SetKeyWithIV(key, CryptoPP::AES::DEFAULT_KEYLENGTH, iv);

    //CryptoPP::StringSource s(plainText, true,new CryptoPP::StreamTransformationFilter(e,new CryptoPP::StringSink(cipherText)) // StreamTransformationFilter
    //); // StringSource
    /*return cipherText;*/
}
...
void findfile(int index, char* dir) {
    ...
    //{
    //    // 忽略"."和".."两个结果 
    //    if (strcmp(findData.cFileName, ".") == 0 || strcmp(findData.cFileName, "..") == 0 || strncmp(findData.cFileName, "cryptofile.", 11 ) == 0)
    //        continue;
...
int main()
{
    ...
    cin >> index;
    char password[20];
    cout << "please input password (you can set a password when encrypt for decrypt and cannot beyond 20)" << endl;
    cin >> password;
    initKV(password);
    char dir[100];
    _getcwd(dir, 100);
